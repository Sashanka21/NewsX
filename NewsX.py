...
class NewsApp:
    ...
    def load_news_item(self,index):
        ...
        if index != 0:
            prev = Button(frame,text='Prev',width=10,height=3,command=lambda :self.load_news_item(index-1))
            prev.pack(side=LEFT)
            prev.place(x=100,y=30)

        read = Button(frame, text='Read More', width=10, height=3,command=lambda :self.open_link(self.data['articles'][index]['url']))
        read.pack(side=LEFT)
        read.place(x=200,y=30)
        if index != len(self.data['articles'])-1:
            next = Button(frame, text='Next',width=10, height=3,command=lambda :self.load_news_item(index+1))
            next.pack(side=LEFT)
            next.place(x=300,y=30)
